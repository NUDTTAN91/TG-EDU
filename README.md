...
| 🤖 **AI智能批改** | 集成DeepSeek大模型，支持自动评分和反馈生成 |
...
| `DEEPSEEK_MODEL` | AI 模型名称 | `deepseek-reasoner` | ❌ |

> 💡 **提示**：生产环境建议使用环境变量或配置文件管理敏感信息
>
> 🤖 **AI功能**：如需使用 AI 自动批改功能，必须配置 `DEEPSEEK_API_KEY`
...
TG-EDU 集成了基于 DeepSeek 大模型的 AI 自动批改系统，支持智能评分和反馈生成，大幅提升教师批改效率。

#### ✨ 主要特性

- **三种批改模式**：
  - 🤖 **仅AI改卷**：完全由AI自动评分和生成反馈
  - 👨‍🏫 **仅教师改卷**：传统教师手动批改模式
  - 🤝 **AI+教师改卷**：AI先评分，教师可修改确认

- **智能文档处理**：
  - 📄 支持 TXT/PDF/DOCX 文档内容提取
  - 🖼️ OCR 图片识别（支持 PDF/Word 中的图片）
  - 📦 压缩包自动解压处理
...
```yaml
environment:
  - DEEPSEEK_API_KEY=your-api-key-here    # 必填：DeepSeek API 密钥
  - DEEPSEEK_MODEL=deepseek-reasoner       # 可选：模型名称
```

> 💡 获取 API Key：访问 [DeepSeek 开放平台](https://platform.deepseek.com/) 注册并创建 API 密钥

**2. 创建作业时启用 AI 批改**

在创建/编辑作业时：
- 选择“AI改卷模式”（仅AI / AI+教师 / 仅教师）
- 填写“参考答案”供AI评分参考
- 设置分数范围
...
- AI 评分仅供参考，建议重要作业使用“AI+教师”模式
- 参考答案质量直接影响 AI 评分准确性
- 默认并发数为 1，可根据 API 配额调整
- 支持的文档格式：TXT、PDF、DOCX、ZIP/RAR
