...
# 临时兼容层 - 保留原有路由，逐步迁移
# ========================================
# 说明: 为了保证系统正常运行，暂时保留原app.py中的部分功能
# TODO: 将以下代码迁移到对应的蓝图中
...
from app.models import (
    User, UserRole, Class, class_student, class_teacher,
    Assignment, AssignmentGrade, Submission, Notification,
    MajorAssignment, Team, TeamMember, TeamInvitation, LeaveTeamRequest
)
...
class Notification(db.Model):
    ...
    notification_type = db.Column(db.String(50), nullable=False)  # 通知类型：system/assignment/grade/team_invitation/leave_request
...
class MajorAssignment(db.Model):
    ...
    requirement_file_path = db.Column(db.String(500))  # 作业要求文件路径
    requirement_file_name = db.Column(db.String(255))  # 作业要求文件名
    requirement_url = db.Column(db.String(500))  # 作业要求链接
    due_date = db.Column(db.DateTime)  # 截止时间
    min_team_size = db.Column(db.Integer, default=2)  # 最小组队人数
    max_team_size = db.Column(db.Integer, default=5)  # 最大组队人数
    class_id = db.Column(db.Integer, db.ForeignKey('class.id'), nullable=False)  # 所属班级
    teacher_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)  # 布置教师
    created_at = db.Column(db.DateTime, default=datetime.utcnow)  # 创建时间
    ...
    teams = db.relationship('Team', backref='major_assignment', cascade='all, delete-orphan')
...
class Team(db.Model):
    ...
    major_assignment_id = db.Column(db.Integer, db.ForeignKey('major_assignment.id'), nullable=False)  # 所属大作业
    leader_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)  # 组长ID
    status = db.Column(db.String(50), default='pending')  # 状态：pending/confirmed/rejected
    size_exception_reason = db.Column(db.Text)  # 人数异常原因（当人数不符合要求时）
    created_at = db.Column(db.DateTime, default=datetime.utcnow)  # 创建时间
    confirmed_at = db.Column(db.DateTime)  # 确认时间
    confirmed_by = db.Column(db.Integer, db.ForeignKey('user.id'))  # 确认者（教师或管理员）
...
class TeamMember(db.Model):
    """团队成员模型"""
    id = db.Column(db.Integer, primary_key=True)
    team_id = db.Column(db.Integer, db.ForeignKey('team.id'), nullable=False)  # 所属团队
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)  # 成员ID
    joined_at = db.Column(db.DateTime, default=datetime.utcnow)  # 加入时间
    
    # 关系
    user = db.relationship('User', backref='team_memberships')
    
    # 唯一约束：一个人在一个团队中只能加入一次
    __table_args__ = (db.UniqueConstraint('team_id', 'user_id', name='unique_team_member'),)
    
    def __repr__(self):
        return f'<TeamMember {self.user.real_name} in Team {self.team_id}>'
...
class TeamInvitation(db.Model):
    ...
    inviter_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)  # 邀请者（组长）
    invitee_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)  # 被邀请者
    status = db.Column(db.String(50), default='pending')  # 状态：pending/accepted/rejected/cancelled
    created_at = db.Column(db.DateTime, default=datetime.utcnow)  # 创建时间
    ...
    def __repr__(self):
        return f'<TeamInvitation from {self.inviter.real_name} to {self.invitee.real_name}>'
...
class LeaveTeamRequest(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    team_id = db.Column(db.Integer, db.ForeignKey('team.id'), nullable=False)  # 团队ID
    member_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)  # 请求退组的成员
    reason = db.Column(db.Text, nullable=False)  # 退组原因
    status = db.Column(db.String(50), default='pending_leader')  # 状态：pending_leader/leader_rejected/pending_teacher/approved/rejected
    created_at = db.Column(db.DateTime, default=datetime.utcnow)  # 创建时间
    leader_responded_at = db.Column(db.DateTime)  # 组长响应时间
    teacher_responded_at = db.Column(db.DateTime)  # 教师响应时间
    reviewer_id = db.Column(db.Integer, db.ForeignKey('user.id'))  # 审批者ID（组长或教师）
    ...
    def __repr__(self):
        return f'<LeaveTeamRequest from {self.member.real_name}>'
...
@app.route('/major_assignments/create', methods=['GET', 'POST'])
@login_required
@require_teacher_or_admin
def create_major_assignment():
    ...
    if request.method == 'POST':
        ...
        if not title or not class_id:
            flash('请填写必填项')
            return redirect(url_for('create_major_assignment'))
        ...
        if requirement_type == 'file':
            ...
            if req_file and req_file.filename:
                ...
                file_path = os.path.join('/app/storage/appendix', filename)
        ...
        flash('大作业布置成功！')
        return redirect(url_for('major_assignment_dashboard'))
...
@app.route('/major_assignments/<int:assignment_id>/teams')
@login_required
@require_teacher_or_admin
...
@app.route('/major_assignments/<int:assignment_id>/student')
@login_required
@require_role(UserRole.STUDENT)
def student_major_assignment_detail(assignment_id):
    ...
    if my_team and my_team.leader_id == current_user.id:
        # 过滤掉已经在团队中的同学
        existing_member_ids = {current_user.id}  # 组长
...
@app.route('/major_assignments/<int:assignment_id>/create_team', methods=['POST'])
@login_required
@require_role(UserRole.STUDENT)
def create_team(assignment_id):
    ...
    for team in major_assignment.teams:
        if team.leader_id == current_user.id:
            flash('您已经是一个团队的组长')
            return redirect(url_for('student_major_assignment_detail', assignment_id=assignment_id))
        for member in team.members:
            if member.user_id == current_user.id:
                flash('您已经加入了一个团队')
                return redirect(url_for('student_major_assignment_detail', assignment_id=assignment_id))
...
def invite_team_members(team_id):
    ...
    if team.leader_id != current_user.id:
        flash('只有组长才能邀请成员')
        return redirect(url_for('student_major_assignment_detail', assignment_id=team.major_assignment_id))
...
@app.route('/teams/<int:team_id>/confirm', methods=['POST'])
@login_required
@require_teacher_or_admin
def confirm_team(team_id):
    ...
    flash('团队已确认')
    return redirect(url_for('view_major_assignment_teams', assignment_id=team.major_assignment_id))

@app.route('/teams/<int:team_id>/reject', methods=['POST'])
@login_required
@require_teacher_or_admin
...
@app.route('/major_assignments/<int:assignment_id>/requirement')
@login_required
...
@app.route('/major_assignments/<int:assignment_id>/edit', methods=['GET', 'POST'])
@login_required
@require_teacher_or_admin
    ...
    if request.method == 'POST':
        ...
        if due_date_str:
            try:
                ...
                flash('日期格式错误')
                return redirect(url_for('edit_major_assignment', assignment_id=assignment_id))
...
@app.route('/team_invitations/<int:invitation_id>/accept', methods=['POST'])
@login_required
def accept_team_invitation(invitation_id):
    ...
    for t in team.major_assignment.teams:
        if t.leader_id == current_user.id:
            flash('您已经有团队了')
...
@app.route('/team_invitations/<int:invitation_id>/reject', methods=['POST'])
@login_required
...
@app.route('/leave_requests/<int:request_id>/approve_by_leader', methods=['POST'])
@login_required
def approve_leave_request_by_leader(request_id):
    ...
    member_to_remove = TeamMember.query.filter_by(
        team_id=team.id,
        user_id=leave_request.member_id
    ).first()
...
@app.route('/leave_requests/<int:request_id>/reject_by_leader', methods=['POST'])
@login_required
...
@app.route('/leave_requests/<int:request_id>/escalate', methods=['POST'])
@login_required
...
@app.route('/leave_requests/<int:request_id>/approve_by_teacher', methods=['POST'])
@login_required
@require_teacher_or_admin
def approve_leave_request_by_teacher(request_id):
    ...
    create_notification(
        ...
        receiver_id=team.leader_id,
        title='团队成员退出',
        content=f'{leave_request.member.real_name} 已退出团队「{team.name}」（教师批准）',
        notification_type='system'
    )
...
@app.route('/leave_requests/<int:request_id>/reject_by_teacher', methods=['POST'])
@login_required
@require_teacher_or_admin
